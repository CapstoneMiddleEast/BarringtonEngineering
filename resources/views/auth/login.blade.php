<x-guest-layout>
    ...
    <div class="pb-5">
        <x-link href="{{ route('home') }}">{{ __('Back to Home!') }}</x-link>
    </div>
    <h2 class="text-balance text-2xl font-semibold tracking-tight dark:text-white text-gray-900 pb-10">To get started,
        Please
        sign in
    </h2>
    <form method="POST" action="{{ route('login') }}">
        ...
        <div class="block mt-4">
            <label for="remember_me" class="inline-flex items-center dark:text-white text-gray-900">
                <input id="remember_me" type="checkbox"
                    class="rounded border-gray-300 text-red-600 shadow-sm focus:ring-red-500" name="remember">
                <span class="ms-2 text-sm dark:text-white text-gray-900">{{ __('Remember me') }}</span>
            </label>
        </div>
