        ...
        <div class="sm:px-6 lg:px-8">
            <div class="bg-gray-100 dark:bg-gray-900 overflow-hidden shadow-sm sm:rounded-lg p-6">
                <div class="flex justify-between items-center">
                    ...
                    <div>
                        <x-btn-red-link href="{{ route('roles.index') }}">
                    ...
                    <form action="{{ route('roles.update', $role->id) }}" method="post">
                        ...
                        <div class="mb-5">
                            ...
                            <x-text-input id="name" name="name" type="text" class="mt-1 block w-full"
                                required autofocus autocomplete="name" value="{{ old('name', $role->name) }}" />
                            <x-input-error class="mt-2" :messages="$errors->get('name')" />
                        </div>
                        <div class="mb-5">
                            <x-input-label for="name" :value="__('Assign permissions')" class="mb-5" />
                            @if ($permissions->isNotEmpty())
                                <ul
                                    class="items-center flex flex-wrap w-full text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg sm:flex dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                                    @foreach ($permissions as $item)
                                        <li class="basis-full md:basis-1/4">
                                            <div class="flex items-center ps-3">
                                                <input type="checkbox" id="item-{{ $item->id }}" name="permission[]" {{ $hasPermissions->contains($item->name) ? 'checked' : '' }}
                                                    value="{{ $item->name }}"
