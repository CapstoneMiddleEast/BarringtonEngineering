                ...
                <div class="flex justify-between items-center">
                    ...
                    <div>
                        @can('create permission')
                            <x-btn-add href="{{ route('permissions.create') }}"></x-btn-add>
                        @endcan
                    </div>
                </div>
            </div>
                                ...
                                @if ($list->isNotEmpty())
                                    @foreach ($list as $item)
                                        <tr
                                            ...
                                            <td class="px-6 py-4">{{ formatted_date($item->created_at) }}</td>
                                            <td class="px-6 py-4 flex">
                                                ...
                                                @can('delete permission')
                                                    <a class="btn btn-circle btn-text btn-sm text-error"
                                                        href="{{ route('permissions.delete', $item->id) }}">
                                                        <x-badge.trash-icon />
                                                    </a>
                                                @endcan
                                            </td>
