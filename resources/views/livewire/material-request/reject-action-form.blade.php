<div>
    <x-red-button data-modal-target="reject-modal" data-modal-toggle="reject-modal">Reject Items</x-red-button>
    <div id="reject-modal" tabindex="-1" data-modal-backdrop="static"
        class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
        <div class="relative p-4 w-full max-w-4xl max-h-full">
            <div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-800">
                <div class="p-4 md:p-5 text-center">
                    <form wire:submit.prevent="saveRejections">
                        <table
                            ...
                            <thead
                                ...
                                <tr>
                                    ...
                                    <th class="px-6 py-3">Rejection Reason (leave empty if not rejected)</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($items as $index => $item)
                                    <tr
                                        ...
                                        <td class="px-6 py-4">
                                            <x-text-area wire:model="items.{{ $index }}.rejected_reason"
                                                class="mt-1 block w-full" placeholder="Enter rejection reason"
                                                rows="1"></x-text-area>
                                        </td>
                                    </tr>
                                @endforeach
                        ...
                        <div class="mt-4 text-right">
                            <x-primary-button data-modal-hide="reject-modal"
                                class="ml-2">{{ __('Save Rejections') }}</x-primary-button>
